...
static void sci_bm_init_name_info(void)
{
	...
	for(bm_chn = 0; bm_chn < bm_st_info.axi_bm_cnt; bm_chn++){
		for(bm_index = 3; bm_index < BM_AXI_TOTAL_CHN_NAME + 3; bm_index++){
			if(bm_chn == *(&bm_st_info.bm_def_mode + bm_index)){
...
static void sci_bm_get_hw_info(void)
{
	...
	if(np){
		...
		if(of_property_read_u32(np, "sprd,bm_status", &val[0])){
			BM_ERR("ERR:it must be sure that the bus monitor is in pref mode!\n");
		...
		if(of_property_read_u32_array(np, "sprd,bm_count", &val[0], 2)){
			BM_ERR("ERR:it must be sure that the there is no count para!\n");
		...
		if(of_property_read_u32(np, "sprd,cpu_chn", &val[0])){
			BM_ERR("ERR:it must be sure that there is not CPU chn!\n");
			bm_st_info.cpu_chn = 0xff;
		}else
			bm_st_info.cpu_chn = val[0];
		if(of_property_read_u32(np, "sprd,disp_chn", &val[0])){
			BM_ERR("ERR:it must be sure that there is not DISP chn!\n");
			bm_st_info.disp_chn = 0xff;
		}else
			bm_st_info.disp_chn = val[0];
		if(of_property_read_u32(np, "sprd,gpu_chn", &val[0])){
			BM_ERR("ERR:it must be sure that there is not GPU chn!\n");
			bm_st_info.gpu_chn = 0xff;
		}else
			bm_st_info.gpu_chn = val[0];
		if(of_property_read_u32(np, "sprd,ap_zip_chn", &val[0])){
			BM_ERR("ERR:it must be sure that there is not AP/ZIP chn!\n");
			bm_st_info.ap_zip_chn = 0xff;
		}else
			bm_st_info.ap_zip_chn = val[0];
		if(of_property_read_u32(np, "sprd,zip_chn", &val[0])){
			BM_ERR("ERR:it must be sure that there is not ZIP chn!\n");
			bm_st_info.zip_chn = 0xff;
		}else
			bm_st_info.zip_chn = val[0];
		if(of_property_read_u32(np, "sprd,ap_chn", &val[0])){
			BM_ERR("ERR:it must be sure that there is not AP chn!\n");
			bm_st_info.ap_chn = 0xff;
		}else
			bm_st_info.ap_chn = val[0];
		if(of_property_read_u32(np, "sprd,mm_chn", &val[0])){
			BM_ERR("ERR:it must be sure that there is not MM chn!\n");
			bm_st_info.mm_chn = 0xff;
		}else
			bm_st_info.mm_chn = val[0];
		if(of_property_read_u32(np, "sprd,cp0_arm0_chn", &val[0])){
			BM_ERR("ERR:it must be sure that there is not CP0 ARM0 chn!\n");
			bm_st_info.cp0_arm0_chn = 0xff;
		}else
			bm_st_info.cp0_arm0_chn = val[0];
		if(of_property_read_u32(np, "sprd,cp0_arm1_chn", &val[0])){
			BM_ERR("ERR:it must be sure that there is not CP0 ARM1 chn!\n");
			bm_st_info.cp0_arm1_chn = 0xff;
		}else
			bm_st_info.cp0_arm1_chn = val[0];
		if(of_property_read_u32(np, "sprd,cp0_dsp_chn", &val[0])){
			BM_ERR("ERR:it must be sure that there is not CP0 DSP chn!\n");
			bm_st_info.cp0_dsp_chn = 0xff;
		}else
			bm_st_info.cp0_dsp_chn = val[0];
		if(of_property_read_u32(np, "sprd,cp0_arm0_1_chn", &val[0])){
			BM_ERR("ERR:it must be sure that there is not CP0 ARM0/1 chn!\n");
			bm_st_info.cp0_arm0_1_chn = 0xff;
		}else
			bm_st_info.cp0_arm0_1_chn = val[0];
		if(of_property_read_u32(np, "sprd,cp1_lte_chn", &val[0])){
			BM_ERR("ERR:it must be sure that there is not CP1 LTE chn!\n");
			bm_st_info.cp1_lte_chn = 0xff;
		}else
			bm_st_info.cp1_lte_chn = val[0];
		if(of_property_read_u32(np, "sprd,cp1_dsp_chn", &val[0])){
			BM_ERR("ERR:it must be sure that there is not CP1 DSP chn!\n");
			bm_st_info.cp1_dsp_chn = 0xff;
		}else
			bm_st_info.cp1_dsp_chn = val[0];
		if(of_property_read_u32(np, "sprd,cp1_arm_chn", &val[0])){
			BM_ERR("ERR:it must be sure that there is not CP1 ARM chn!\n");
			bm_st_info.cp1_arm_chn = 0xff;
		}else
			bm_st_info.cp1_arm_chn = val[0];
		if(of_property_read_u32(np, "sprd,cp2_chn", &val[0])){
			BM_ERR("ERR:it must be sure that there is not CP2 chn!\n");
