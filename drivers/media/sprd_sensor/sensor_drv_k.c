...
LOCAL unsigned int _sensor_k_get_id_pin(uint32_t *fd_handle)
{
...
#ifndef	CONFIG_SC_FPGA
	fd->gpio_tab.detect_id = of_get_gpio(p_mod->of_node, 8+fd->sensor_id);
	gpio_direction_input(fd->gpio_tab.detect_id);
	pin_status = (gpio_get_value(fd->gpio_tab.detect_id) ? 1 : 0);
	SENSOR_PRINT_HIGH("SENSOR: detect id(%d) status =  \n", fd->gpio_tab.detect_id, pin_status);
#endif
	...
	case SENSOR_IO_DETECT_ID_PIN:
		{
			...
			id_pin = _sensor_k_get_id_pin((uint32_t *)p_file);
				...
				if (INTERFACE_OPEN == if_cfg.is_open) {
					if (INTERFACE_MIPI == if_cfg.if_type) {
						if (0 == p_file->mipi_on) {
							...
							udelay(500);
							ret = csi_reg_isr(_sensor_csi2_error, (void*)p_file);
...
int sensor_k_probe(struct platform_device *pdev)
{
	...
	for (i = 0; i < SENSOR_DEV_MAX; i++) {
		...
		gpio_tab.detect_id = of_get_gpio(p_mod->of_node, 8+i);
		ret = gpio_request(gpio_tab.detect_id, NULL);
		if (ret) {
			tmp = 1;
			printk("sensor:  gpio already request reset %d %d.\n", gpio_tab.detect_id, i);
		}
	}
#endif
