...
extern const int g_TpVirtualKey_Exchange[];
#endif
...
//    DBG("*** %s() ***\n", __func__);
	...
	if (0 == _DrvFwCtrlParsePacket(pPacket, nReportPacketLength, &tInfo))
	{
		...
		{
			if(nLastTouchKeyCode != 0)
			{
				for(i=0; i<MAX_KEY_NUM; i++)
				{
					if(nLastTouchKeyCode & (1<<i))
					{
#ifdef CTP_EXCHANGE_KEY
						if(MsKeyIsExchange())
						{
							nTouchKeyCode =  g_TpVirtualKey_Exchange[i];
						}
						else
#endif
						{
							nTouchKeyCode = g_TpVirtualKey[i];
						}

						printk("nTouchKeyCode = %d, key touch released\n", nTouchKeyCode);

						input_report_key(g_InputDevice, BTN_TOUCH, 0);
						input_report_key(g_InputDevice, nTouchKeyCode, 0);
					}
				}
		...
		else //touch on screen
		{
			...
			{
#ifdef CONFIG_TP_HAVE_KEY
				tInfo.nTouchKeyCode &= 0x7;
				if(tInfo.nTouchKeyCode != nLastTouchKeyCode)
				{
					unsigned int i;

					for(i=0; i<MAX_KEY_NUM; i++)
					{
						if((tInfo.nTouchKeyCode & (1<<i)) != (nLastTouchKeyCode & (1<<i)))
						{
#ifdef CTP_EXCHANGE_KEY
							if(MsKeyIsExchange())
							{
							...
							printk("nTouchKeyCode = %d ", nTouchKeyCode);

							if(tInfo.nTouchKeyCode & (1<<i))
							{
								printk("key touch pressed\n");
					...
					input_sync(g_InputDevice);
					nLastTouchKeyCode = tInfo.nTouchKeyCode;
				}
#endif //CONFIG_TP_HAVE_KEY
			}
			else
